...
describe('LeadsManager', () => {
  ...
  describe('submitLead', () => {
    it('creates and returns the new lead', async () => {
      ...
      const dto: any = { name: 'Alice', phone: '0501111111', concern: 'כאב גב' };
    ...
    it('passes optional fields to service', async () => {
      const dto: any = { name: 'Alice', phone: '0501111111', concern: 'כאב גב', email: 'alice@example.com', notes: 'הערה' };
  ...
  describe('updateStatus', () => {
    it('updates status from new to contacted', async () => {
      ...
      const result = await manager.updateStatus('l1', { status: 'contacted' } as any);
    ...
    it('updates status to booked', async () => {
      ...
      const result = await manager.updateStatus('l1', { status: 'booked' } as any);
      expect(result.status).toBe('booked');
    });

    it('updates notes on a lead', async () => {
      mockService.update.mockResolvedValueOnce({ ...lead1, notes: 'תיאום בוצע' });
      const result = await manager.updateStatus('l1', { notes: 'תיאום בוצע' } as any);
