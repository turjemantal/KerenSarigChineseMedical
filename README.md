...
- **50-minute minimum spacing** — every appointment occupies a 50-minute window (`APPOINTMENT_DURATION_MINUTES` — the *single* duration in the system: slot spacing, block overlap, Google Calendar event length, the user-facing error text, and the length shown to clients all read it). The client can't import from the server (separate Docker build contexts), so `client/src/constants.ts` mirrors it — and that mirror is **guarded twice**: `server/tests/shared-constants.spec.ts` fails CI if the two copies diverge in the repo, and the client compares its build-time value against `appointmentDurationMinutes` from `GET /api/clinic-settings/public` at runtime (`client/src/utils/publicSettings.ts`), catching the case CI can't — client and server ship as separate images, so one can be rolled back without the other. The rule is enforced in three layers so an admin schedule change can never create an overlapping double-booking: (1) `computeAvailableSlots` hides any candidate time within 50 minutes of an already-taken time (exactly 50 minutes apart stays bookable); (2) saving the weekly schedule rejects two day-times less than 50 minutes apart; (3) opening an extra slot rejects a time within 50 minutes of a base weekly time or another extra slot on that date
...
- **Admin dashboard** — lead pipeline, appointment management (with a name/phone search box alongside the status filter tabs), calendar (week view on desktop, day agenda on mobile), fully usable from a phone
- **Data-driven calendar viewport** — the admin calendar derives its grid from the schedule instead of hard-coding one: the week starts on **Sunday** and renders Sun–Thu plus Friday/Saturday whenever either carries weekly hours, an extra slot, or an appointment; the hour range starts at 08:00–20:00 and **expands** to cover the earliest/latest time actually in use (`client/src/utils/calendarViewport.ts`). Opening a new day or an unusually early/late hour in the weekly board can therefore never produce a booking that renders off-grid and looks missing
- **Automated reminders** — an hourly cron (clinic time, Asia/Jerusalem) sends reminders for the *next day's* confirmed appointments at the admin-configured hour (default 09:00); the reminder carries the appointment time **and the clinic address** (`CLINIC_ADDRESS`, the single source also used for the Google Calendar event location). A failed send is left unmarked, and the admin can re-send a reminder for any appointment from its detail drawer
- **Health check** — public `GET /api/health` reports app + DB status (the sanctioned read-only way to verify production)
- **Google Calendar sync** — when configured (service account), a calendar event is created the moment a client books, marked "ממתין לאישור" (awaiting approval) in its title so a pending request is visible in the calendar right away; on approve, the event is rewritten to the normal (non-pending) title; on reject, cancel (pending or scheduled), or admin hard-delete, the event is removed; on reschedule, the event is moved. Existing appointments backfilled via the on-demand script `scripts/v3/backfill-google-calendar.ts`; separately, clients get a client-side "Add to Google Calendar" link on the booking confirmation screen and in the portal next to every upcoming appointment (pending or scheduled) — no longer sent via SMS
